...
GIVEAWAY_FILE = "giveaway_data.pkl"
...
class DrawProgressData:
    def __init__(self, id, user_id, chanel_id, chanel_name, text, file_type, file_id, winers_count, post_time, end_time):
        self.id = id; self.user_id = user_id; self.chanel_id = chanel_id; self.chanel_name = chanel_name
        self.text = text; self.file_type = file_type; self.file_id = file_id
        self.winers_count = winers_count; self.post_time = post_time; self.end_time = end_time
...
class DrawData:
    def __init__(self, id, user_id, message_id, chanel_id, chanel_name, text, file_type, file_id, winers_count, post_time, end_time):
        self.id = id; self.user_id = user_id; self.message_id = message_id; self.chanel_id = chanel_id
        self.chanel_name = chanel_name; self.text = text; self.file_type = file_type; self.file_id = file_id
        self.winers_count = winers_count; self.post_time = post_time; self.end_time = end_time

class SubscribeChannelData:
    def __init__(self, draw_id, user_id, channel_id):
        self.draw_id = draw_id; self.user_id = user_id; self.channel_id = channel_id

class DrawPlayerData:
    def __init__(self, draw_id, user_id, user_name):
        self.draw_id = draw_id; self.user_id = user_id; self.user_name = user_name

class GiveawayDB:
    def __init__(self):
        self.draw_progress: List[DrawProgressData] = []
        self.draw_not: List[DrawNotData] = []
        self.draws: List[DrawData] = []
        self.subscribe_channels: List[SubscribeChannelData] = []
        self.draw_players: List[DrawPlayerData] = []
        self._next_id = 1
        self.load()
    ...
    def _get_next_id(self):
        while True:
            yield self._next_id
            self._next_id += 1
    _id_gen = property(lambda self: self._get_next_id())

    def select_all(self, cls, **filter_s):
        if cls == DrawProgressData: lst = self.draw_progress
        elif cls == DrawNotData: lst = self.draw_not
        elif cls == DrawData: lst = self.draws
        elif cls == SubscribeChannelData: lst = self.subscribe_channels
        elif cls == DrawPlayerData: lst = self.draw_players
        else: return []
        if not filter_s: return lst
        result = []
        for item in lst:
            ok = True
            for k, v in filter_s.items():
                if getattr(item, k, None) != v: ok = False; break
            if ok: result.append(item)
        return result

    def get_one(self, cls, **filter_s):
        res = self.select_all(cls, **filter_s)
        return res[0] if res else None

    def new(self, cls, *args, **kwargs):
        if cls == DrawProgressData:
            obj = DrawProgressData(*args, **kwargs)
            self.draw_progress.append(obj)
        elif cls == DrawNotData:
            obj = DrawNotData(*args, **kwargs)
            self.draw_not.append(obj)
        elif cls == DrawData:
            obj = DrawData(*args, **kwargs)
            self.draws.append(obj)
        elif cls == SubscribeChannelData:
            obj = SubscribeChannelData(*args, **kwargs)
            self.subscribe_channels.append(obj)
        elif cls == DrawPlayerData:
            obj = DrawPlayerData(*args, **kwargs)
            self.draw_players.append(obj)
        else: return None
        self.save()
        return obj

    def delete(self, cls, **filter_s):
        if cls == DrawProgressData: lst = self.draw_progress
        elif cls == DrawNotData: lst = self.draw_not
        elif cls == DrawData: lst = self.draws
        elif cls == SubscribeChannelData: lst = self.subscribe_channels
        ...
        before = len(lst)
        lst[:] = [x for x in lst if not all(getattr(x, k) == v for k, v in filter_s.items())]
        if len(lst) != before:
            self.save()
            return True
        return False
    ...
    def save(self):
        data = {
            ...
            "draw_not": self.draw_not,
            "draws": self.draws,
            "subscribe_channels": self.subscribe_channels,
            "draw_players": self.draw_players,
            "next_id": self._next_id,
        }
        with open(GIVEAWAY_FILE, "wb") as f: pickle.dump(data, f)

    def load(self):
        if Path(GIVEAWAY_FILE).exists():
            with open(GIVEAWAY_FILE, "rb") as f:
                data = pickle.load(f)
                self.draw_progress = data.get("draw_progress", [])
                self.draw_not = data.get("draw_not", [])
                self.draws = data.get("draws", [])
                self.subscribe_channels = data.get("subscribe_channels", [])
                self.draw_players = data.get("draw_players", [])
...
@dp.message(ContestStates.waiting_for_photo)
...
async def process_end(message: Message, state: FSMContext):
    ...
    data = await state.get_data()
    giveaway_db.delete(DrawProgressData, user_id=str(message.from_user.id))
    new_id = next(giveaway_db._id_gen)
    giveaway_db.new(DrawProgressData, new_id, str(message.from_user.id), data['chanel_id'], data['chanel_name'],
                    data['draw_text'], data['file_type'], data['file_id'], data['winers_count'],
                    data['start_time'], data['end_time'])
    dp_db = giveaway_db.get_one(DrawProgressData, user_id=str(message.from_user.id))
...
async def publish_contest(callback: CallbackQuery):
    dp_db = giveaway_db.get_one(DrawProgressData, user_id=str(callback.from_user.id))
    if dp_db:
        giveaway_db.new(DrawNotData, dp_db.id, dp_db.user_id, dp_db.chanel_id, dp_db.chanel_name, dp_db.text,
                        dp_db.file_type, dp_db.file_id, dp_db.winers_count, dp_db.post_time, dp_db.end_time)
        giveaway_db.delete(DrawProgressData, user_id=str(callback.from_user.id))
...
async def my_contests(callback: CallbackQuery):
    notposted = giveaway_db.select_all(DrawNotData, user_id=str(callback.from_user.id))
    posted = giveaway_db.select_all(DrawData, user_id=str(callback.from_user.id))
...
async def join_contest(callback: CallbackQuery):
    ...
    draw = giveaway_db.get_one(DrawData, id=draw_id)
    if not draw: await callback.answer("Розыгрыш не найден", show_alert=True); return
    channels = giveaway_db.select_all(SubscribeChannelData, draw_id=draw.id)
    for ch in channels:
        try:
            ...
            await callback.answer(TEXTS['draw']['not_subscribe'], show_alert=True); return
    existing = giveaway_db.get_one(DrawPlayerData, draw_id=str(draw.id), user_id=str(callback.from_user.id))
    if existing: await callback.answer(TEXTS['draw']['already_in'], show_alert=True); return
    giveaway_db.new(DrawPlayerData, draw.id, str(callback.from_user.id), str(callback.from_user.username))
    count = len(giveaway_db.select_all(DrawPlayerData, draw_id=str(draw.id)))
...
async def add_fake_players(message: Message):
    ...
    for i in range(count):
        giveaway_db.new(DrawPlayerData, draw_id, str(ADMIN_ID) + "_fake_" + str(i), "fake_user_" + str(i))
    total_players = len(giveaway_db.select_all(DrawPlayerData, draw_id=str(draw_id)))
    try:
        if draw.message_id:
...
async def start_draw_timer():
    while True:
        for item in giveaway_db.select_all(DrawNotData):
            ...
            if now_t >= post_t:
                ...
                try:
                    ...
                    giveaway_db.new(DrawData, item.id, item.user_id, str(msg.message_id), item.chanel_id, item.chanel_name,
                                    item.text, item.file_type, item.file_id, item.winers_count, item.post_time, item.end_time)
                    giveaway_db.delete(DrawNotData, id=item.id)
                except Exception as e:
                    logging.error(f"Ошибка публикации {item.id}: {e}")
        await asyncio.sleep(5)

async def end_draw_timer():
    while True:
        for item in giveaway_db.select_all(DrawData):
            now_str = datetime.now().strftime('%Y-%m-%d %H:%M')
            now_t = time.strptime(now_str, '%Y-%m-%d %H:%M')
            end_t = time.strptime(item.end_time, '%Y-%m-%d %H:%M')
            if now_t >= end_t:
                players = giveaway_db.select_all(DrawPlayerData, draw_id=str(item.id))
                if not players:
                ...
                try:
                    ...
                    giveaway_db.delete(DrawData, id=item.id)
